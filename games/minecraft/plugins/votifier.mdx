...
To have voting sites listen for votes through the plugin, you need to have an open port that can be accessed by the site available. You can do this by heading into the `Network` tab on the panel, and press the "**Create Allocation**" button to create a port.
...
Aaand voilà! You now have voting and rewards functionality setup on your Minecraft server! If you need help, reach out to us on our [Community Discord](https://discord.gg/compute) or contact us via [email](mailto:support@octant.sh) or livechat.