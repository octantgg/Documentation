...
This guide will walk you through installation and usage of Dynmap on your [Minecraft server](https://octant.sh/games/minecraft)!
...
If you have a [Minecraft server from Octant](https://octant.sh/games/minecraft), we've simplified all steps below to a few buttons on our [Panel](https://my.octant.sh), where you can easily set up a reverse proxy for Dynmap using the [Proxies Manager](panel/proxies) tool. We take care of SSL certificates and all the configuration for you.
...
```bash dynmap.conf
curl -s https://cdn.octant.sh/scripts/dynmap.sh | sudo bash
```
...
If you continue to experience issues with Dynmap, you can ask for help on our [Discord server](https://discord.gg/compute)!